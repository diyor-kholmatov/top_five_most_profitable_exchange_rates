...
async def get_currency_exchange():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        currency_rate_content = soup.find_all("div", id="best_EUR")
        currency_dict = {}
        for currency_r in currency_rate_content:
            currency_name = 'EUR'
            currency_purchase = [item.text.strip() for item in currency_r.find(class_='bc-inner-block-left').find_all(
                class_='bc-inner-block-left-texts')]
            currency_sale = [item1.text.strip() for item1 in currency_r.find(class_='bc-inner-blocks-right').find_all(
                class_='bc-inner-block-left-texts')]
    ...
    with open('scrapingfile/euro_exchange_rate.json', 'w', encoding='utf-8') as file:
